...
libraries_to_import = [
    "colorama", "requests", "tls_client", "pystyle", "datetime", "httpx"
]
...
ctypes.windll.kernel32.SetConsoleTitleW(
    f'[ Discord Promo Checker ] By tigerltc ~ ')


def update_title():
    global total, valid, invalid, redeemed
    ctypes.windll.kernel32.SetConsoleTitleW(
        f'[ Discord Promo Checker ] | Valid : {valid} | Invalid : {invalid} | Redeemed : {redeemed} | Working Rate : {round(valid/total*100,2)}% | github.com/H4cK3dR4Du'
    )
...
def check_promo(promo):
    ...
    proxy = random.choice(open("proxies.txt", "r").readlines()).strip() if len(
    ...
    try:
        ...
        if "You are being rate limited." in r.text:
            ...
            time_rn = get_time_rn()
            print(
                f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({red}-{gray}) {pretty}Invalid {gray}|{pink} https://promos.discord.gg/{reset}{promo}"
            )
            invalid += 1
            total += 1
            update_title()
            ...
            if check_redeemed == False:
                ...
                if max_uses != uses:
                    promotion = r_json['promotion'][
                        'inbound_header_text'].split('T')[0]
                    expiry = str(promotion)
                    time_rn = get_time_rn()
                    print(
                        f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({green}+{gray}) {pretty}Valid {gray}|{pink} https://promos.discord.gg/{reset}{promo}"
                    )
                    valid += 1
                    total += 1
                    update_title()
                    with open("Results/valid_promo.txt",
                              "a+",
                              encoding='utf-8') as f:
                    ...
                    time_rn = get_time_rn()
                    print(
                        f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({yellow}/{gray}) {pretty}Redeemed {gray}|{pink} https://promos.discord.gg/{reset}{promo}"
                    )
                    redeemed += 1
                    total += 1
                    update_title()
